...
ym_vl_re=apple.com
echo
green "准备调用Acme-yg脚本申请域名证书..."
yellow "请确保您有一个解析到本VPS IP地址的域名"
sleep 2
bash <(curl -Ls https://raw.githubusercontent.com/yonggekkk/acme-yg/main/acme.sh)
if [[ ! -f /root/ygkkkca/cert.crt || ! -f /root/ygkkkca/private.key || ! -s /root/ygkkkca/cert.crt || ! -s /root/ygkkkca/private.key ]]; then
red "Acme证书申请失败！脚本将退出。" 
red "请检查域名解析或更换域名后重试。"
exit 1
else
green "Acme证书申请成功！"
blue "Vmess-ws、Hysteria-2、Tuic-v5将使用 $(cat /root/ygkkkca/ca.log 2>/dev/null) 证书，并开启SNI证书验证"
tlsyn=true
...
tls=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[1].tls.enabled')
[[ "$tls" = "false" ]] && vm_na="当前已关闭TLS。$ymzs ${yellow}将开启TLS${plain}" || vm_na="正在使用的域名证书：$(cat /root/ygkkkca/ca.log 2>/dev/null)。$yellow切换为关闭TLS$plain"
hy2_sniname=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.inbounds[2].tls.key_path')
...
unset swd4 swd6 ssd4 ssg4 ssd6 ssg6 sad4 sag4 sad6 sag6
...
sfl4="${yellow}【$warp_s4_ip】已分流：$ssd4${plain} "
fi

ad4=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.route.rules[4].domain_suffix | join(" ")')
if [[ "$ad4" == "yg_kkk" ]]; then
...
adfl4="${yellow}【$vps_ipv4】已分流：$sad4${plain} "
fi

ad6=$(sed 's://.*::g' /etc/s-box/sb.json | jq -r '.route.rules[5].domain_suffix | join(" ")')
if [[ "$ad6" == "yg_kkk" ]]; then
...
adfl6="${yellow}【$vps_ipv6】已分流：$sad6${plain} "
fi
}
...
blue "socks5需要在VPS安装warp官方客户端或者WARP-plus-Socks5-赛风VPN (选项1)"
blue "VPS本地出站分流(选项2与3)"
echo
blue "当前Sing-box内核不支持geosite分流方式，仅支持完整域名方式"
echo
yellow "注意："
yellow "一、完整域名方式只能填完整域名 (例：谷歌网站填写：www.google.com)"
yellow "二、同一个完整域名切勿重复分流"
yellow "三、如分流通道中有个别通道无网络，所填分流为黑名单模式，即屏蔽该网站访问"
changef
}
...
sbymfl
echo
green "1：重置warp-socks5-ipv4优先分流域名 $sfl4"
green "2：重置VPS本地ipv4优先分流域名 $adfl4"
green "3：重置VPS本地ipv6优先分流域名 $adfl6"
green "0：返回上层"
echo
readp "请选择【0-3】：" menu
...
sed -i "145s/.*/$s4flym/" /etc/s-box/sb.json
restartsb
changef
...
sed -i "151s/.*/$ad4flym/" /etc/s-box/sb.json
restartsb
changef
...
sed -i "157s/.*/$ad6flym/" /etc/s-box/sb.json
restartsb
changef
else
sb
...
if [[ "$tls" = "false" ]]; then
echo -e "🚀【   Vmess-ws    】${yellow}端口:$vm_port   证书形式:$vm_zs${plain}"
else
echo -e "🚀【 Vmess-ws-tls  】${yellow}端口:$vm_port   证书形式:$vm_zs${plain}"
fi
...
clear
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
white "Vless-reality-vision、Vmess-ws-tls、Hysteria-2、Tuic-v5 四协议共存脚本"
white "脚本快捷方式：sb"
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
green " 1. 一键安装 Sing-box" 
green " 2. 删除卸载 Sing-box"
white "----------------------------------------------------------------------------------"
green " 3. 变更配置 【证书/UUID路径/IP优先/TG通知/订阅/CDN优选】" 
