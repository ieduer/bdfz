...
inssb(){
    ...
    if [[ ! -f '/etc/s-box/sing-box.tar.gz' || ! -s '/etc/s-box/sing-box.tar.gz' ]]; then red "下載內核失败"; exit 1; fi
...
apply_acme_cert() {
    ...
    local prev_domain=""; [[ -s "/root/ygkkkca/ca.log" ]] && prev_domain=$(cat /root/ygkkkca/ca.log 2>/dev/null || true)
...
insport() {
    red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    green "三、设置各个协议端口"
    ports=(); for i in {1..4}; do while true; do local p=$(shuf -i 10000-65535 -n 1); if ! [[ " ${ports[@]} " =~ " $p " ]] && ! ss -H -tunlp "sport = :$p" | grep -q .; then ports+=("$p"); break; fi; done; done
    ...
    port_vm_ws=${cdn_ports[$RANDOM % ${#cdn_ports[@]}]}; while ss -H -tunlp "sport = :$port_vm_ws" | grep -q . || [[ " ${ports[@]} " =~ " $port_vm_ws " ]]; do port_vm_ws=${cdn_ports[$RANDOM % ${#cdn_ports[@]}]}; done
...
inssbjsonser(){
    ...
    local vmess_tls_alpn=""; if [[ "${tlsyn}" == "true" ]]; then vmess_tls_alpn=', "alpn": ["http/1.1"]'; fi
...
"outbounds": [ { "type":"direct", "tag":"direct", "domain_strategy": "$dns_strategy" }, { "type": "block", "tag": "block" } ],
...
post_install_check() {
    green "執行安裝後檢查..."; if ! /etc/s-box/sing-box check -c "/etc/s-box/sb.json"; then red "❌ 配置文件語法錯誤！"; return 1; fi
...
display_sharing_info() {
    ...
    local vl_port=$(echo "$config" | jq -r '.inbounds[] | select(.tag=="vless-sb") | .listen_port'); local vl_sni=$(echo "$config" | jq -r '.inbounds[] | select(.tag=="vless-sb") | .tls.server_name'); local vl_short_id=$(echo "$config" | jq -r '.inbounds[] | select(.tag=="vless-sb") | .tls.reality.short_id[0]'); local vl_link="vless://$uuid@$server_ipcl:$vl_port?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$vl_sni&fp=chrome&pbk=$public_key&sid=$vl_short_id&type=tcp&headerType=none#vl-reality-$hostname"; echo "$vl_link" > /etc/s-box/vl_reality.txt
...
install_or_reinstall() {
    ...
    uuid=$(/etc/s-box/sing-box generate uuid); key_pair=$(/etc/s-box/sing-box generate reality-keypair)
    private_key=$(echo "$key_pair" | awk '/PrivateKey/ {print $2}' | tr -d '"'); public_key=$(echo "$key_pair" | awk '/PublicKey/ {print $2}' | tr -d '"')
    echo "$public_key" > /etc/s-box/public.key; short_id=$(/etc/s-box/sing-box generate rand --hex 4)
...
stclre(){ 
    ...
    if [[ x"${release}" == x"alpine" ]]; then case "$act" in 1) rc-service sing-box restart;; 2) rc-service sing-box stop;; 3) rc-service sing-box start;; esac
...
main_menu() {
    ...
    red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
    if [[ -f '/etc/s-box/sing-box' ]]; then 
        local corev; corev=$($(/etc/s-box/sing-box version 2>/dev/null || echo "version unknown") | awk '/version/{print $NF}'); 
    ...
    case "$Input" in  
     1 ) install_or_reinstall;; 2 ) unins;; 3 ) install_or_reinstall;; 4 ) stclre;;
