#!/usr/bin/env bash
# Seiue Auto Attendance - One-click Installer (Apiall mode) - v1.0 Robust Edition
# OS: macOS/Homebrew-friendly, also works on Linux
# Generates ~/.seiue-auto-attendance with a venv and your Apiall-based saa.py
...
# ----------------- NEW: Pre-flight Environment Checks -----------------
check_environment() {
  info "--- 正在執行環境預檢 ---"
  local all_ok=true
...
# ----------------- MODIFIED: Setup Environment -----------------
setup_environment() {
  info "正在設定工作目錄: ${INSTALL_DIR}"

  # MODIFIED: Non-destructive reinstall logic
  ...
  # --- 写入 Python 脚本 ---
  info "生成 Apiall 版 Python 腳本..."
  TMP_PY="$(mktemp)"
  cat > "$TMP_PY" <<'EOF_PYTHON'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
if __name__ == "__main__":
    main()
EOF_PYTHON
  install -m 0644 -o "$REAL_USER" -g "$(id -gn "$REAL_USER")" "$TMP_PY" "${INSTALL_DIR}/${PYTHON_SCRIPT_NAME}"
  rm -f "$TMP_PY"
  success "Python 腳本已生成（Apiall 版）。"
}
...
main() {
  ...
  echo -e "${C_GREEN}--- Seiue 自動考勤一鍵安裝 (v1.0) ---${C_RESET}"
  
  # NEW: Call the pre-flight check function first
  check_environment
